...
echo "操作完成"
deepspeed --include localhost:${localhost} ha_dpo/models/llava-v1_5/train_dpo_post.py \
    --lora_enable False \
...
output_dir2="ha_dpo/models/llava-v1_5/checkpoints/llava-post-decoder-${time2}-${version2}-bs-${train_bs2}-${eval_bs2}-${gradient_accumulation_steps2}-epoch-${epoch2}-gpu-${gpu2}-lr-${lr2}"
...
# output_dir3="ha_dpo/models/llava-v1_5/checkpoints/llava-post-decoder-${time3}-${version3}-bs-${train_bs3}-${eval_bs3}-${gradient_accumulation_steps3}-epoch-${epoch3}-gpu-${gpu3}-lr-${lr3}"
...
# output_dir4="ha_dpo/models/llava-v1_5/checkpoints/llava-post-decoder-${time4}-${version4}-bs-${train_bs4}-${eval_bs4}-${gradient_accumulation_steps4}-epoch-${epoch4}-gpu-${gpu4}-lr-${lr4}"
...
CUDA_VISIBLE_DEVICES=${localhost} torchrun --nproc_per_node ${gpu1} --master_port $RANDOM ha_dpo/models/llava-v1_5/pope_eval_post.py \
