...
- **vision_tower**: vision encoder, 属于 linear 层, 在 HA-DPO 代码中要训练
...
我们需要指定显卡, 指定预训练文件位置和保存文件位置, 设置哪里参数会被更新, 还需要设置 batch_size 和 梯度保存 steps. 所以我们作出以下调整:
...
3. `DataCollatorForSupervisedDataset` 类为 supervised fine-tuning 准备和组织训练数据
...
6. `maybe_zero_3` 函数使用 ZeRO3 的量化策略, 用以减小显存使用

7. `get_peft_state_maybe_zero_3` 函数获取训练后的参数

8. `get_peft_state_non_lora_maybe_zero_3` 函数获取训练后非 lora 的模型参数
...
(TODO: check) SaverCallBack: 将 on_train_end 函数的 `if isinstance(kwargs['model'], PeftModelForCausalLM)` 改为 `if isinstance(kwargs['model'], LlavaLlamaForCausalLM)`. 这样可以成功保存 lm_head 的参数. 在 `ha_dpo/models/llava-v1_5/checkpoints/llava-origin` 路径下, 可以看到 `config.json`, `generation_config.json`, `non_lora_trainables.bin`, `pytorch_model.bin` 四个文件. 其中 `non_lora_trainables.bin` 包括 lm_head 更新后的参数


#### **base_dpo_trainer_origin.py**
...
#### **llava_llama.py** 文件中加入post progress module (unfinished)
...
`LlavaLlamaForCausalLM` 类继承自 `LlamaForCausalLM` 和 `LlavaMetaForCausalLM`. 该类的初始化与其父类 `LlamaForCausalLM` 相同, 父类是 transformers 库中的模块. 在 `train_dpo.py` 文件中, 以下代码定义了该类的初始化输入
