...
class PIBSelfAttention(nn.Module):
    ...
    def forward(
        ...
        causal_mask = torch.triu(torch.ones(tgt_len, src_len), diagonal=1).to(attn_weights.device)
        causal_mask = causal_mask.masked_fill(causal_mask == 1, float('-inf'))
        
        # Adding causal_mask to attn_weights
...
class PIBSATransformerBlock(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, intermediate_size, hidden_act, attention_dropout=0.0, layer_norm_eps=1e-5):
        super().__init__()
        self.embed_dim = hidden_size
        self.self_attn = PIBSelfAttention(hidden_size, num_attention_heads, attention_dropout=attention_dropout)
...
class PostInteractionBlock(nn.Module):
    ...
    def __init__(self, config: LlamaPIBConfig, depth=1):
        ...
        self.blocks = nn.ModuleList([
            PIBSATransformerBlock(config.hidden_size, config.num_attention_heads, config.intermediate_size, config.hidden_act)
