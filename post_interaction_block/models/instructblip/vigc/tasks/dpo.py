...
class DPOTask(BaseTask):
    ...
    def build_model(self, cfg):
        ...
        # freeze qformer in DPO training
        freeze_llm_proj = model_config.get("freeze_llm_proj", True)
        if freeze_llm_proj:
            for name, param in model.llm_proj.named_parameters():
                param.requires_grad = False

        # freeze llm_proj in DPO training
        freeze_q_former = model_config.get("freeze_q_former", True)
        if freeze_q_former:
