...
@registry.register_model("dpo_blip2_vicuna_instruct_post")
class DPOBlip2VicunaInstructPIB(Blip2Base):
    """
    BLIP2 Vicuna model.
    ...
    def __init__(
        ...
        from vigc.models_post.blip2_models.modeling_llama_post import LlamaPIBForCausalLM
        ...
        #     llm_model, torch_dtype=torch.float16, device_map=device_map
        # )
        
        # self.llm_model = LlamaPIBForCausalLM.from_pretrained(
        #     llm_model, torch_dtype=torch.bfloat16, device_map=device_map
        # )
    ...
    def generate(
        ...
        if "prefix" in samples:
            prefix_ = samples["prefix"]
            prompt = f"{prompt} {prefix_}".strip() 
