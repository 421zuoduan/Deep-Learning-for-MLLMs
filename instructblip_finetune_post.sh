...
target_folder1="/home/cuiruochen/model/InstructBLIP/vicuna-7b-v1.1-post-${time1}-${version1}-bs-${train_bs1}-${eval_bs1}-${gradient_accumulation_steps1}-epoch-${epoch1}-gpu-${gpu1}-lr-${lr1}"
output_dir1="/home/cuiruochen/model/InstructBLIP/vicuna-7b-v1.1-post-${time1}-${version1}-bs-${train_bs1}-${eval_bs1}-${gradient_accumulation_steps1}-epoch-${epoch1}-gpu-${gpu1}-lr-${lr1}"
# 检查目标文件夹路径是否正确
...
accelerate launch --main_process_port $RANDOM post_interaction_block/models/instructblip/train_instructblip_dpo_post.py \
    --lora_r 64 \
